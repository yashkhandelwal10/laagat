...
//   double _totalGreenExpense = 0.0;
//   double _totalMonthlyExpense = 0.0; // Added for total monthly expense
//   double _totalFilteredDaysExpense =
//       0.0; // Added for total filtered days expense
...
//             ),
//             Text(
//               'Total Filtered Days Expense: ${_totalFilteredDaysExpense.toStringAsFixed(2)}', // Display total filtered days expense
//               style: TextStyle(fontWeight: FontWeight.bold),
//             ),
//             Expanded(
//               child: _messages.isNotEmpty
...
//       double greenExpense = 0.0;
//       double monthlyExpense = 0.0;
//       double filteredDaysExpense = 0.0; // Initialize filtered days expense
...
//           // Calculate total filtered days expense
...
//               _selectedFilter == day.toString() &&
//               month == _selectedMonth &&
//               year == _selectedYear) {
//             filteredDaysExpense += double.parse(trimBody(message.body!)) ?? 0.0;
//           }
//         }
//       }
...
//                 month == _selectedMonth &&
//                 year == _selectedYear;
//           } else if (_selectedFilter == 'Exclude') {
...
//         _totalGreenExpense = greenExpense;
//         _totalMonthlyExpense = monthlyExpense; // Update total monthly expense
//         _totalFilteredDaysExpense =
//             filteredDaysExpense; // Update total filtered days expense
//       });
//     } else {
//       await Permission.sms.request();
//     }
...
//     _initializeMonthsList();
//     _initializeYearsList();
//     _fetchSMS(); // Fetch SMS messages when the screen initializes
//   }
...
//             ),
//             if (_selectedFilter !=
//                 'Exclude') // Show total filtered days expense if filter is not excluded
//               Text(
...
//           // Calculate total filtered days expense based on selected filter
//           if (_selectedFilter != 'Exclude') {
//             var day = message.date!.day;
//             var month = DateFormat('MMMM').format(message.date!);
//             var year = DateFormat('yyyy').format(message.date!);
...
//       // Filter messages based on selected month, year, and filter criteria
...
//         // Update total filtered days expense
//         if (_selectedFilter != 'Exclude') {
    ...
    if (permission.isGranted) {
      ...
      for (var message in messages) {
        if (_containsKeyword(message.body!) && !_containsOTP(message.body!)) {
          ...
          // _categoryExpenses[category]! += double.parse(trimBody(message.body!) ?? '0.0');
          // _categoryExpenses.forEach((category, expense) {
          //   print('Category: $category, Expense: $expense');
          // });
